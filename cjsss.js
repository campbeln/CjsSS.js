/** ################################################################################################
 * @class CjsSS
 * @classdesc Use Javascript In Your CSS
 * @version 0.10.2018-03-22
 * @author Nick Campbell
 * @license MIT
 * @copyright 2014-2018, Nick Campbell (cjsssdev@gmail.com, http://opensourcetaekwondo.com/cjsss/)
 * @desc Add in a library such as Chroma (https://github.com/gka/chroma.js) to get color functionality.
################################################################################################# */
!function /*cjs3Core*/(_window, _document, fnEvalerMetaFactory, fnImplementation) {
    'use strict';
    ...
    var fnPrevaler,
        ...
        $baseServices = {
                    ...
                    //# Else we need to build the oCache entry for this _element
                        ...
                        switch (_element.tagName.toLowerCase()) {
                            case "style": {
                                ...
                                //# 
                                _element.type = "text/css"; //# _element.setAttribute("type", "text/css");
                                //_element.setAttribute(cjsss, "");
                                break;
                            }
                            case "link": {
                                bIsLink = true;
            ...
            runScripts: function (oCacheEntry, oCompiledOptions, oInject) {
                ...
                //#     TODO: Move this if block out into cjs3Core?
    ...
    $baseServices.evalFactory = fnEvalerMetaFactory(_window, _document, $baseServices);

    //# Now that we have everything orchestrated, run the fnImplementation functionality
    fnImplementation(_window, _document, $baseServices);
}(
    //# Pass in the standard objects (code-golf/DI)
    window,
    document,
    ...
    function /*fnEvalerMetaFactory*/(fnEvalerFactory, fnLocalEvaler, fnUseStrictEvaler, fnSandboxEvalerFactory /*, $services*/) {
        return function (_window, _document, $services) {
            //# Build the passed fnEvalerFactory
            //#     NOTE: The fnLocalEvaler is specifically placed outside of the "use strict" block to allow for the local eval calls below to persist across eval'uations
            return fnEvalerFactory(_window, _document, $services, fnLocalEvaler, fnUseStrictEvaler, fnSandboxEvalerFactory);
        };
    }(
        //# <EvalerJS>
        ...
        function /*fnEvalerFactory*/(_window, _document, $services, fnLocalEvaler, fnUseStrictEvaler, fnSandboxEvalerFactory) {
            "use strict";
    ...
    //# Define the CjsSS functionality
    //#     NOTE: Everything above this is cjs3Core and EvalerJS functionality which is maintained elsewhere (e.g. boilerplate code). The code below actually implements CjsSS.
    function /*CjsSSFactory*/(jQuery) {
        return function /*CjsSS*/(_window, _document, $services) {
            'use strict';
            ...
            var cjsss = "cjsss",
                ...
                $cjsss = {
                    ...
                    options: {                                                  //# STRING (CSS Selector); See: http://lesscss.org/#client-side-usage , http://stackoverflow.com/questions/7731702/is-it-possible-to-inline-less-stylesheets
                        ...
                        optionScope: "json",                                    //# STRING (enum: json, global, local, object, sandbox); Javascript scope to evaulate attributes within.
                        scope: "isolated",                                      //# STRING (enum: global, local, object, sandbox); Javascript scope to evaluate code within.
                        expose: cjsss,                                          //# STRING; Target under window to expose cjsss?
                ...
                js: {
                    ...
                    autorun: function () {
                        ...
                        //# Else we'll need to .expose ourselves (otherwise the developer won't have access our functionality)
                            ...
                            //oDefaultOptions.expose = cjsss;
                            $services.js.expose(oDefaultOptions.expose);
                        }
                    }, //# autorun
                    ...
                    expose: function (sTarget) {
                        ...
                        if (!bExposed) {
                            //# Parse the sTarget into its path segments (defaulting it to cjsss if its not .is.str) and flip bExposed
                            a_sTarget = ($services.is.str(sTarget) ? sTarget : cjsss).split(".");
                            bExposed = true;
    
                            //# Traverse the a_sTarget segments, creating each as an object if its not already .is.obj or .is.fn
                            for (i = 0; i < a_sTarget.length; i++) {
                                //# If this segment .is.obj or .is.fn, reset oCurrent to it
                                if ($services.is.obj(oCurrent[a_sTarget[i]], true)) {
                                    oCurrent = oCurrent[a_sTarget[i]];
                                }
                                //# Else force this segment into an object
                                else {
                                    oCurrent = oCurrent[a_sTarget[i]] = {};
                                }
                            }
    
                            //# .extend oCurrent with the internal $cjsss, resetting both to the new .extend'ed object
                            //#     NOTE: oDefaultOptions and $services are .extended in the procedural code below, so there is no need to so it again here
                            $cjsss = oCurrent = $services.extend(oCurrent, $cjsss);
                    ...
                    process: function (vElements, oProcessOptions) {
                        ...
                        if (oPrelimOptions.expose) {
                            $services.js.expose(oPrelimOptions.expose);
                        }
    
                        //# Traverse the a__ElementsToCompile (if any), excluding SCRIPT .tagName's as we go
                        for (i = 0; i < a__ElementsToCompile.length; i++) {
                            if (a__ElementsToCompile[i].tagName !== "SCRIPT") {
                    ...
                    conf: function () {
                        ...
                        //# If the developer has setup a _window[cjsss] object
                        ...
                        //# Else attempt to .parse our SCRIPT[cjsss] .getAttribute, .warn'ing the user if an error occurs
                        else {
                            try {
                                $services.extend(oDefaultOptions,
                                    JSON.parse(
                                        (_document.currentScript || $services.js.dom("SCRIPT[" + cjsss + "]")).getAttribute(cjsss) || null
                                    )
                                );
                            } catch (e) {
                                $services.warn("JSON options under SCRIPT[" + cjsss + "] could not be imported.");
                            }
                        }
